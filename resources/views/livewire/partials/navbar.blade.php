<nav x-data="{ open: false, activeLink: '{{ request()->path() }}' }" 
    x-init="$watch('activeLink', value => $wire.call('setActiveLink', value))"
    class="bg-white shadow-sm">
   <div id="loading-bar" class="h-1 bg-[#00dffd] fixed top-0 left-0 z-50" style="width: 0%;"></div>

   <!-- Pantalla de carga con spinner -->
   <div id="loading-screen" class="fixed inset-0 bg-white bg-opacity-75 z-50 flex items-center justify-center ">
       <div class="animate-spin rounded-full h-32 w-32 border-t-2 border-b-2 border-[#1d4570]"></div>
   </div>
       ...
       <div class="flex justify-between h-20">
               ...
               @foreach(['home' => 'Inicio', 'agenda' => 'Agenda', 'inscribirte' => 'Inscribirte', 'contacto' => 'Contacto'] as $route => $label)
                   <a href="{{ $route === 'home' ? route('home') : ($route === 'contacto' ? route('contacto') : $route) }}"
                      ...
                      :class="{'text-[#1d4570] font-semibold': activeLink === '{{ $route }}'}">
                       {{ $label }}
                       <span class="absolute bottom-0 left-0 w-full h-0.5 bg-[#00dffd] transform scale-x-0 transition-transform duration-200 ease-in-out"
           ...
           <div class="-mr-2 flex items-center sm:hidden">
               <button x-on:click="open = !open" type="button"
                   ...
                   <svg class="h-6 w-6" x-bind:class="{ 'hidden': open, 'block': !open }" xmlns="http://www.w3.org/2000/svg"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                       <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                   </svg>
                   <svg class="h-6 w-6" x-bind:class="{ 'block': open, 'hidden': !open }" xmlns="http://www.w3.org/2000/svg"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
